...
async def state_image_news(message: types.Message, state: FSMContext):
    ...
    await message.reply('Новость успешна создана!', reply=False)
    await state.finish()
...
async def create_schedule(message: types.Message):
    ...
    await message.reply('Выберите класс, где вы хотите поменять расписание:', reply=False,
...
async def state_select_group_schedule(message: types.Message, state: FSMContext):
    ...
    if message.text in all_groups_names:
        ...
        await message.reply('Теперь отправь фото расписания:', reply=False,
                            reply_markup=types.ReplyKeyboardRemove())
        await states.ScheduleStates.next()
    else:
        await bot.send_message(message.chat.id, 'Такой класс не найден.')
        await state.finish()
...
async def state_image_schedule(message: types.Message, state: FSMContext):
    ...
    await message.reply('Расписание успешно добавлено!', reply=False)
    async with state.proxy() as data:
        await send_schedule.send_schedule(data['group'])
    await state.finish()
...
async def delete_schedule(message: types.Message):
    ...
    await message.reply('Выберите класс, расписание которого хотите убрать:', reply=False,
                        reply_markup=kb)
